...
def talk_added(sender, instance=None, **kwargs):
    if notification and kwargs['created']:
        users = [ii.user for ii in notification.NoticeSetting.objects.filter(notice_type__label='talk_added')]
...
def talk_accepted(sender, instance=None, **kwargs):
    if notification and not kwargs['created'] and instance.accepted:
        users = [ii.user for ii in notification.NoticeSetting.objects.filter(notice_type__label='talk_accepted')]
